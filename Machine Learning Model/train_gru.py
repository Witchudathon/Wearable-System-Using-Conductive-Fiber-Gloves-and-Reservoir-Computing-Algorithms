...
# -*- coding: utf-8 -*-
"""
train_gru.py — GRU Training (NOT Bidirectional)
═════════════════════════════════════════════════
Standard uni-directional GRU with 2 layers (128→64),
...
# ═══════════════════════════════════════════════════════════════
#  MODEL BUILDER — Standard GRU (NO Bidirectional)
# ═══════════════════════════════════════════════════════════════

def build_gru(n_timesteps, n_features, n_classes):
    import tensorflow as tf
    tf.random.set_seed(SEED)
    model = tf.keras.Sequential([
        tf.keras.layers.Input(shape=(n_timesteps, n_features)),
        tf.keras.layers.GRU(DL_UNITS1, return_sequences=True,
                            kernel_regularizer=tf.keras.regularizers.l2(1e-4)),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(DL_DROPOUT),
        tf.keras.layers.GRU(DL_UNITS2, return_sequences=False,
                            kernel_regularizer=tf.keras.regularizers.l2(1e-4)),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(DL_DROPOUT),
...
def train_gru_model(df_ts, enhanced_cols, file_groups, classes, rounds):
    import tensorflow as tf
    tf.random.set_seed(SEED)
    ...
    for r in range(rounds):
        ...
        model = build_gru(DL_WINDOW, len(enhanced_cols), n_classes)
        ...
        print(f"    GRU Round {r+1}/{rounds}  (round acc={round_acc:.2%})", flush=True)
    ...
    if last_history:
        ...
        fig.suptitle("GRU — Training History (Last Round)", fontsize=16, fontweight="bold")
        plt.tight_layout(); fig.savefig(out / "training_history.png"); plt.close(fig)

    acc, _ = save_model_outputs("GRU", out, cm_agg,
        y_true_all, y_pred_all, classes,
        {"architecture": f"GRU({DL_UNITS1})→GRU({DL_UNITS2})",
...
if __name__ == "__main__":
    t0 = time.time()
    print("=" * 70)
    print(f"  GRU Training — GRU({DL_UNITS1})→GRU({DL_UNITS2}), 30ch, Prob Voting")
    print("=" * 70)
    ...
    print(f"\n  [3/3] Training GRU ...", flush=True)
    acc, round_accs = train_gru_model(df_ts, enhanced_cols, file_groups, classes, rounds)

    # Save round metrics
    metrics = {"GRU": {"overall_accuracy": acc, "round_accuracies": round_accs}}
    mf = BASE_OUT / "gru" / "round_metrics.json"
    mf.write_text(json.dumps(metrics, indent=2, default=float), encoding="utf-8")

    elapsed = time.time() - t0
    print(f"\n{'='*70}")
    print(f"  GRU Accuracy: {acc:.2%}  (elapsed: {elapsed/60:.1f} min)")
